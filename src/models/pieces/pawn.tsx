...
class Pawn extends Piece {
    ...
    public getEnPassantMove(
        ...
        if (
            Math.abs(
                lastOpponentMove.oldPosition.row -
                    lastOpponentMove.newPosition.row
            ) !== 2
        ) {
            return undefined;
        }
        ...
        if (
            Math.abs(opponentPawnPosition.col - this.currentPosition.col) !== 1
        ) {
            return undefined;
        }
        ...
        if (
            board.pieces[enPassantPosition.row][enPassantPosition.col] !==
            undefined
        ) {
            return undefined;
        }
    ...
    public getKillMoves(board: Board, history: Move[]): Map<string, Position> {
        const leftDiagonal: Position = new Position(
            this.currentPosition.row + this.getCardinality(),
            this.currentPosition.col - 1
        );
        const rightDiagonal: Position = new Position(
            this.currentPosition.row + this.getCardinality(),
            this.currentPosition.col + 1
        );
        ...
        const leftDiagonalContainsOpponentPiece =
            board.pieces[leftDiagonal.row][leftDiagonal.col] instanceof Piece;
        const leftDiagonalContainsOpponentColour =
            board.pieces[leftDiagonal.row][leftDiagonal.col]?.colour !==
            this.colour;
        if (
            leftDiagonalWithinBoard &&
            leftDiagonalContainsOpponentPiece &&
            leftDiagonalContainsOpponentColour
        ) {
            killMoves.set(leftDiagonal.key(), leftDiagonal);
        }

        const rightDiagonalWithinBoard = Utils.isWithinBoard(rightDiagonal);
        const rightDiagonalContainsOpponentPiece =
            board.pieces[rightDiagonal.row][rightDiagonal.col] instanceof Piece;
        const rightDiagonalContainsOpponentColour =
            board.pieces[rightDiagonal.row][rightDiagonal.col]?.colour !==
            this.colour;
        if (
            rightDiagonalWithinBoard &&
            rightDiagonalContainsOpponentPiece &&
            rightDiagonalContainsOpponentColour
        ) {
    ...
    public evaluateMoves(
        ...
        const killMoves: Map<string, Position> = this.getKillMoves(
            board,
            history
        );
        ...
        let p: Position = new Position(
            this.currentPosition.row + this.getCardinality() * OTHER_MOVES,
        ...
        if (this.currentPosition.isEqual(this.initialPosition)) {
            ...
            if (
                board.pieces[p.row][p.col] === undefined &&
                forwardMoves.size > 0
            ) {
                // if there is a piece in front, we can't move 2 steps
                forwardMoves.set(p.key(), p);
            }
        }

        let toReturn: Map<string, Position> = new Map<string, Position>([
            ...killMoves,
            ...forwardMoves,
        ]);
        if (log) {
