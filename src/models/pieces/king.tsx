...
class King extends Piece{
    ...
    public getCastlingPosition(board: Board, rook: Rook, isInCheck: boolean): Position | undefined {
        // check if king moved
        if(this.hasMoved()) {
        ...
        const moveDirection = rook.currentPosition.col > this.currentPosition.col ? 1 : -1;
        let startCol = this.currentPosition.col + moveDirection;
        while(startCol !== rook.currentPosition.col) {
            if(board.pieces[this.currentPosition.row][startCol] instanceof Piece) {
                return undefined;
            }
        ...
        let opponentPossibleMoves = board.getOpponentKills(Utils.switchColour(this.colour), []);

        const kingCastleMoves: Map<string, Position> = new Map<string, Position>();
        let p: Position = new Position(this.currentPosition.row, this.currentPosition.col + (moveDirection * 2));
        kingCastleMoves.set(p.key(), p);
        p = new Position(this.currentPosition.row, this.currentPosition.col + (moveDirection * 1));
        kingCastleMoves.set(p.key(), p);
        ...
        return new Position(this.currentPosition.row, this.currentPosition.col + (moveDirection * 2));
    }

    public getCastlingMoves(board: Board, isInCheck: boolean): Map<string, Position>{
        ...
        for (let rook of rooks) {
            const castlePosition = this.getCastlingPosition(board, rook, isInCheck);
            if(castlePosition) {
