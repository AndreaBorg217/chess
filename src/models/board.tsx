...
class Board {
    ...
    constructor() {
        this.pieces = Array.from({ length: ROWS }, () =>
            Array(COLUMNS).fill(undefined)
        );
        this.backgroundColours = Array.from({ length: ROWS }, () =>
            Array(COLUMNS).fill("")
        );
        this.clickable = Array.from({ length: ROWS }, () =>
        ...
        this.pieces.forEach((row) => {
            console.log(
                row
                    .map((piece) =>
                        piece
                            ? `${piece.name.toUpperCase()}_${piece.colour.charAt(0)}`
                            : "[]"
                    )
                    .join("\t")
            );
    ...
    public setBoardColours(): void {
        for (let row = 0; row < ROWS; row++) {
            for (let col = 0; col < COLUMNS; col++) {
                this.backgroundColours[row][col] =
                    (row + col) % 2 === 0 ? BOARD_GREEN : BOARD_WHITE;
            }
        }
    }
    ...
    public getKing(colour: Colour): King | undefined {
        for (let row = 0; row < ROWS; row++) {
            for (let col = 0; col < COLUMNS; col++) {
                if (
                    this.pieces[row][col] instanceof King &&
                    this.pieces[row][col]?.colour === colour
                ) {
    ...
    public getRooks(colour: Colour): Rook[] {
        ...
        for (let row = 0; row < ROWS; row++) {
            for (let col = 0; col < COLUMNS; col++) {
                if (
                    this.pieces[row][col] instanceof Rook &&
                    this.pieces[row][col]?.colour === colour
                ) {
    ...
    public getOpponentKills(
        colour: Colour,
        history: Move[] = []
    ): Map<string, Position> {
        let opponentPossibleMoves: Map<string, Position> = new Map<
            string,
            Position
        >();
        for (let row = 0; row < ROWS; row++) {
            for (let col = 0; col < COLUMNS; col++) {
                if (
                    this.pieces[row][col] instanceof Piece &&
                    this.pieces[row][col]?.colour === colour
                ) {
