...
export default function Turn({ controller }: { controller: GameController }) {
    return(
        <div id="turn-container">
            {Array.from(controller.gameStates.values()).some(state => 
            state === GameState.CHECKMATE || state === GameState.STALEMATE
            ) ? (
                <button id="restart-button" onClick={() => controller.restartGame()}>
